...
<head>
  ...
  <style>
   ...
   .cls2{text-align:center; font-size:30px; height: 20px;}
  </style>
		...
		<div id="section">
			...
			<h3>
				...
				<div id="middle" class="fl">
					...
					<div class="fl">검색</div>
						<form>
							<input type="text" placeholder="검색" name="keyword" value="${keyword }" class="fl searchInput"/>
							<input type="submit" class="fl searchButton" value=""/>
						</form>
					</div>
				...
				<div id="low" class="fl cf">
					...
					<c:when test="${!empty articlesList}" >
					   ...
					   <div>
						<div class="boardContentNo fl" width="5%">${data.num}</div>
						<div class="boardContentWriter ellipsis fl" width="10%">${data.writer }</div>
					...
					<div class="cls2 ">
					<c:forEach var="row" items="${data }"> <!-- begin="1" end="11" step="1" -->
						<div class="num">${row.num }</div>
						<div class="category ellipsis">${row.writer }</div>
						<div class="title"><a href="/project03/detail?num=${row.num }">${row.title }</a></div>
						<div class="date ellipsis"><fmt:formatDate value="${row.insert_date}" pattern="yyyy.MM.dd"/></div>
					</c:forEach>
					
					 
					</div>    
					<br><br>
					<a  class="writingText writingBtn"  href="/project03/create">글쓰기</a>
	</section>
